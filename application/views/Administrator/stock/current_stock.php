				...
				<table class="table table-bordered" 
					...
					<tfoot>
						<tr>
							...
							<th>{{ stock.reduce((prev, curr) => { return prev + parseFloat(curr.purchased_quantity)}, 0) | decimal }}</th>
							<th>{{ stock.reduce((prev, curr) => { return prev + parseFloat(curr.purchase_returned_quantity)}, 0) | decimal }}</th>
							<th>{{ stock.reduce((prev, curr) => { return prev + parseFloat(curr.damaged_quantity)}, 0) | decimal }}</th>
							<th>{{ stock.reduce((prev, curr) => { return prev + parseFloat(curr.sold_quantity)}, 0) | decimal }}</th>
							<th>{{ stock.reduce((prev, curr) => { return prev + parseFloat(curr.sales_returned_quantity)}, 0) | decimal }}</th>
							<th>{{ stock.reduce((prev, curr) => { return prev + parseFloat(curr.transferred_to_quantity)}, 0) | decimal }}</th>
							<th>{{ stock.reduce((prev, curr) => { return prev + parseFloat(curr.transferred_from_quantity)}, 0) | decimal }}</th>
							<th>{{ stock.reduce((prev, curr) => { return prev + parseFloat(curr.current_quantity)}, 0) | decimal }}</th>
							<th>{{ stock.reduce((prev, curr) => { return prev + parseFloat(curr.Product_Purchase_Rate)}, 0) | decimal }}</th>
							<th>{{ totalStockValue | decimal }}</th>
						</tr>
					</tfoot>
				</table>
